...
#read tree file
tree <- read.tree(file = "Liv_ST744.tree")
...
#TREE SPECIFIC INDEX
rowname_replace <- c(rowname_replace[1:7],"Reference")
...
phylodata <- rbind(phylodata, rep(NA, ncol(phylodata)))

phylodata$phyloname <- rowname_replace

genotype_wsums[is.na(genotype_wsums)] <- 'REF'

tree <- root(tree, outgroup = 'Reference')

p <- ggtree(tree) %<+% 
  phylodata +
  geom_tiplab(size = 2, offset = 0.075) +
...
LivST744 <- gheatmap(p = p, data = genotype_wsums,
                    colnames_offset_y = -0.48,
                    font.size = 2,
                    hjust = 0,
